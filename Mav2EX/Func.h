...
void setHomeVars()
{
 ...
 //0 = no GPS, 1 = nofix, 2=2D , 3=3D
  ...
  if(osd_got_home == 0 && osd_fix_type > 2){
    ...
    if(osd_alt_cnt < 35){
      if(fabs(osd_alt_prev - osd_press_alt) > 0.5){
        osd_alt_cnt = 0;
        osd_alt_prev = osd_press_alt;
      }
      else
      {
        if(++osd_alt_cnt >= 35){
          osd_home_alt = osd_press_alt;  // take this stable osd_alt as osd_home_alt
        }
      }
    }
    ...
    if (osd_home_alt != -9) // we have it
      ...
      //  osd_alt = osd_baro_alt - osd_home_alt; osd_baro_alt is nowhere set, therefore removed
        osd_home_altdif = osd_press_alt - osd_home_alt;
      }
