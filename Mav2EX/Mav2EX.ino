...
#ifndef DEBUG // Daten aus MAVLink.ino
...
void setup()
{
  ...
  JB.setValue6(JB.addData(F("Arm/Dis"), F("")), &motor_armed, 0); // 0 = Disarmed : 1 = Armed   Pos.01 ###OK###

  JB.setValue6(JB.addData(F("Mode"), F("")), &osd_mode, 0); // Betriebsmodus     Pos.02 ###OK###
  JB.setValue6(JB.addData(F("GPS Lock"), F("")), &osd_fix_type_jeti, 0); // GPS no : 2D : 3D   Pos.03 ###OK###
  JB.setValue6(JB.addData(F("GPS Sat"), F("")), &osd_satellites_visible, 0); // Anzahl Satelliten empfangen  Pos.04 ###OK###

  //JB.setValue14(JB.addData(F("Batt %"), F("%")), &osd_battery_remaining_A, 0); // Verbleibende Batteriekapazit�t in Prozent vom Autopilot
  JB.setValue14(JB.addData(F("GCourse"), F("Deg")), &osd_heading, 0); // Gesteuerter Grundkurs Pos. 05  ###OK###
  JB.setValue14(JB.addData(F("Home Dir"), F("Deg")), &osd_home_heading, 0);   //   Kurs nach Hause vom Konverter   Pos.06  ###OK###
  //JB.setValue14(JB.addData(F("HDOP"), F("")), &ap_gps_hdop, 2);
  
  JB.setValue14(JB.addData(F("Airspeed"), F("m/s")), &osd_airspeed, 0);  // Geschwindigkeit durch die Luft  Pos.07  ###OK###
  JB.setValue14(JB.addData(F("Vario"), F("m/s")), &osd_climb, 0);  // Variometer  Pos.08  ###OK###
  JB.setValue14(JB.addData(F("Throttle"), F("%")), &osd_throttle, 0);  // F�llung in Prozent vom Autopilot  Pos.09  ###OK###

  /*
  JB.setValue14(JB.addData(F("Pitch"), F("Deg")), &osd_pitch, 0);
  JB.setValue14(JB.addData(F("Roll"), F("Deg")), &osd_roll, 0);
  */
  JB.setValue14(JB.addData(F("Alt-AGL"), F("m")), &osd_home_altdif, 0);  // H�he AGL  Pos.10  ###OK###
  JB.setValue14(JB.addData(F("Batt V"), F("V")), &osd_vbat_A, 1);  // Batterie  Spannung   Pos.11 ###OK###
  JB.setValue14(JB.addData(F("Batt A"), F("A")), &osd_curr_A, 1);  // Batterie Strom Pos.12 ###OK###
  JB.setValue30(JB.addData(F("Batt C"), F("mAh")), &osd_current_consumed, 0);  // Batterie mA verbraucht Pos.13  ###OK###
  ...
  JB.setValueGPS(JB.addData( F("Lat"), F("")), &osd_lat, false); //Pos.14   ###OK###
  JB.setValueGPS(JB.addData( F("Lon"), F("")), &osd_lon, true);  //Pos.15   ###OK###
