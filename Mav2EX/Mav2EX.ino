...
void setup()
{
  ...
  //JB.setValue14(JB.addData(F("Batt %"), F("%")), &osd_battery_remaining_A, 0); // Verbleibende Batteriekapazität in Prozent vom Autopilot
  ...
  JB.setValue14(JB.addData(F("Throttle"), F("%")), &osd_throttle, 0);  // Füllung in Prozent vom Autopilot  Pos.09  ###OK###

  /*
  JB.setValue14(JB.addData(F("Pitch"), F("Deg")), &osd_pitch, 0);
  JB.setValue14(JB.addData(F("Roll"), F("Deg")), &osd_roll, 0);
  */
  JB.setValue14(JB.addData(F("Alt-AGL"), F("m")), &osd_alt_agl, 0);  // Höhe AGL  Pos.10  ###OK###
